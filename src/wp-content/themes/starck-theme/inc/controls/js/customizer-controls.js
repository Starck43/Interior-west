( function( api ) {
	...
	api( 'starck_settings[nav_position_setting]', function( setting ) {
		...
		api.control( 'starck_settings[nav_drop_point]', linkSettingValueToControlActiveState );
	} );

	var setOption = function( headerAlignment, navLocation, navAlignment ) {
		if ( headerAlignment ) {
			api.control( 'starck_settings[header_alignment_setting]' ).setting.set( headerAlignment );
		}

		if ( navLocation ) {
			api.control( 'starck_settings[nav_position_setting]' ).setting.set( navLocation );
		}

		if ( navAlignment ) {
			api.control( 'starck_settings[nav_alignment_setting]' ).setting.set( navAlignment );
		}
	};

	api( 'starck_header_helper', function( value ) {
		var headerAlignment = false,
			navLocation = false,
			navAlignment = false;

		value.bind( function( newval ) {
			var headerAlignmentSetting = api.control( 'starck_settings[header_alignment_setting]' ).setting;
			var navLocationSetting = api.control( 'starck_settings[nav_position_setting]' ).setting;
			var navAlignmentSetting = api.control( 'starck_settings[nav_alignment_setting]' ).setting;
			...
			if ( 'default' === newval ) {
				setOption( starck_defaults.header_alignment_setting, starck_defaults.nav_position_setting, starck_defaults.nav_alignment_setting );
			}
	...
	api( 'nav_color_presets', function( value ) {
		...
		value.bind( function( newval ) {
			var backgroundColorSetting = api.instance( 'starck_settings[navigation_background_color]' ),
				textColorSetting = api.instance( 'starck_settings[navigation_text_color]' ),
				backgroundColorHoverSetting = api.instance( 'starck_settings[navigation_background_hover_color]' ),
				textColorHoverSetting = api.instance( 'starck_settings[navigation_text_hover_color]' ),
				currentBackgroundColorSetting = api.instance( 'starck_settings[navigation_background_current_color]' ),
				currentTextColorSetting = api.instance( 'starck_settings[navigation_text_current_color]' ),
				subMenuBackgroundColorSetting = api.instance( 'starck_settings[subnavigation_background_color]' ),
				subMenuTextColorSetting = api.instance( 'starck_settings[subnavigation_text_color]' ),
				subMenuBackgroundColorHoverSetting = api.instance( 'starck_settings[subnavigation_background_hover_color]' ),
				subMenuTextColorHoverSetting = api.instance( 'starck_settings[subnavigation_text_hover_color]' ),
				subMenuCurrentBackgroundColorSetting = api.instance( 'starck_settings[subnavigation_background_current_color]' ),
				subMenuCurrentTextColorSetting = api.instance( 'starck_settings[subnavigation_text_current_color]' );
			...
			if ( 'default' === newval ) {
				backgroundColorSetting.set( starck_color_defaults.navigation_background_color );
				textColorSetting.set( starck_color_defaults.navigation_text_color );

				backgroundColorHoverSetting.set( starck_color_defaults.navigation_background_hover_color );
				textColorHoverSetting.set( starck_color_defaults.navigation_text_hover_color );

				currentBackgroundColorSetting.set( starck_color_defaults.navigation_background_current_color );
				currentTextColorSetting.set( starck_color_defaults.navigation_text_current_color );

				subMenuBackgroundColorSetting.set( starck_color_defaults.subnavigation_background_color );
				subMenuTextColorSetting.set( starck_color_defaults.subnavigation_text_color );

				subMenuBackgroundColorHoverSetting.set( starck_color_defaults.subnavigation_background_hover_color );
				subMenuTextColorHoverSetting.set( starck_color_defaults.subnavigation_text_hover_color );

				subMenuCurrentBackgroundColorSetting.set( starck_color_defaults.subnavigation_background_current_color );
				subMenuCurrentTextColorSetting.set( starck_color_defaults.subnavigation_text_current_color );
			}
