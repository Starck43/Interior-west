wp.customize.controlConstructor['starck-range-slider'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
		    ...
		    inputDefault,
		    changeAction,
			controlClass = '.customize-control-starck-range-slider',
			footerActions = jQuery( '#customize-footer-actions' );
		
		// Set up the sliders
		jQuery( '.starck-slider' ).each( function() {
		...
		jQuery( controlClass + ' .gp_range_value input[type=number]' ).on( 'input', function() {
			...
			jQuery( this ).closest( 'label' ).find( '.starck-slider' ).slider( 'value', parseFloat(value)).change();
		});

		// Handle the reset button
		jQuery( controlClass + ' .starck-reset' ).on( 'click', function() {
			var icon = jQuery( this ),
				...
				slider_value = visible_area.find( '.starck-slider' ),
		...
		jQuery( controlClass + ' .starck-range-slider-control' ).each( function() {
			var _this = jQuery( this );
