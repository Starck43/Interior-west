( function( api ) {
	...
	api.controlConstructor['gp-customizer-typography'] = api.Control.extend( {
		ready: function() {
			...
			control.container.on( 'change', '.starck-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the starck_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'starck_get_all_google_fonts_ajax',
						...
						if ( id in fonts ) {
							...
							jQuery( '.starck-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.starck-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.starck-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.starck-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.starck-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.starck-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.starck-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.starck-font-variant' );
			...
			control.container.on( 'change', '.starck-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.starck-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.starck-font-transform select',
				function() {
...
jQuery( document ).ready( function( $ ) {
	...
	$( '.starck-font-family select' ).select2();

	$( '.starck-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".starck-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.starck-font-variant' ).hide();
		}
	} );
