...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Starck_Range_Slider_Control' ) ) {
	/**
	 ...
	 * @since 1.3.47
	 */
	class Starck_Range_Slider_Control extends WP_Customize_Control {
		/**
		 * The control type.
		 *
		 * @access public
		 * @var string
		 */
		public $type = 'starck-range-slider';
		...
		public function to_json() {
			...
			$this->json['desktop_label'] = __( 'Desktop','starck' );
			$this->json['tablet_label'] = __( 'Tablet','starck' );
			$this->json['mobile_label'] = __( 'Mobile','starck' );
			$this->json['reset_label'] = __( 'Reset','starck' );
		...
		public function enqueue() {
			wp_enqueue_script( 'starck-range-slider', trailingslashit( get_template_directory_uri() ) . 'inc/controls/js/slider-control.js', array( 'jquery', 'customize-base', 'jquery-ui-slider' ), false, true );
			wp_enqueue_style( 'starck-range-slider-css', trailingslashit( get_template_directory_uri() ) . 'inc/controls/css/slider-customizer.css', null );
		}
		...
		protected function content_template() {
			?>
			<div class="starck-range-slider-control">
					...
					<div class="gp-range-slider-controls">
						<span class="gp-device-controls">
							<# if ( 'undefined' !== typeof ( data.desktop ) ) { #>
								<span class="starck-device-desktop dashicons dashicons-desktop" data-option="desktop" title="{{ data.desktop_label }}"></span>
							<# } #>

							<# if ( 'undefined' !== typeof (data.tablet) ) { #>
								<span class="starck-device-tablet dashicons dashicons-tablet" data-option="tablet" title="{{ data.tablet_label }}"></span>
							<# } #>

							<# if ( 'undefined' !== typeof (data.mobile) ) { #>
								<span class="starck-device-mobile dashicons dashicons-smartphone" data-option="mobile" title="{{ data.mobile_label }}"></span>
							<# } #>
						</span>

						<span title="{{ data.reset_label }}" class="starck-reset dashicons dashicons-image-rotate"></span>
					</div>
				</div>
					...
					<# if ( 'undefined' !== typeof ( data.desktop ) ) { #>
						<label class="range-option-area" data-option="desktop" style="display: none;">
							<div class="wrapper <# if ( '' !== data.choices['desktop']['unit'] ) { #>has-unit<# } #>">
								...
								<div class="starck-slider" data-step="{{ data.choices['desktop']['step'] }}" data-min="{{ data.choices['desktop']['min'] }}" data-max="{{ data.choices['desktop']['max'] }}"></div>
							</div>
						</label>
					<# } #>
					...
					<# if ( 'undefined' !== typeof ( data.tablet ) ) { #>
						<label class="range-option-area" data-option="tablet" style="display:none">
							<div class="wrapper <# if ( '' !== data.choices['tablet']['unit'] ) { #>has-unit<# } #>">
								...
								<div class="starck-slider" data-step="{{ data.choices['tablet']['step'] }}" data-min="{{ data.choices['tablet']['min'] }}" data-max="{{ data.choices['tablet']['max'] }}"></div>
							</div>
						</label>
					<# } #>
					...
					<# if ( 'undefined' !== typeof ( data.mobile ) ) { #>
						<label class="range-option-area" data-option="mobile" style="display:none;">
							<div class="wrapper <# if ( '' !== data.choices['mobile']['unit'] ) { #>has-unit<# } #>">
								...
								<div class="starck-slider" data-step="{{ data.choices['mobile']['step'] }}" data-min="{{ data.choices['mobile']['min'] }}" data-max="{{ data.choices['mobile']['max'] }}"></div>
							</div>
						</label>
					<# } #>
				</div>
